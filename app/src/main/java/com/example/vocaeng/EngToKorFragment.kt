...
class EngToKorFragment(category: String) : Fragment() {
    ...
    private fun initData(i:Int) {
        ...
        binding!!.apply {
            question.text=data[i].eng
            when(random.nextInt(3)){
                0 ->{
                    radioButton.text=data[i].kor
                    val str=getInCorrect(data[i].eng)
                    radioButton2.text=str[0]
                    radioButton3.text=str[1]
                    answer=0
                }
                1 ->{
                    radioButton2.text=data[i].kor
                    val str=getInCorrect(data[i].eng)
    ...
    fun getInCorrect(eng:String):ArrayList<String>{
        val str=ArrayList<String>()
        while(str.size!=2){
            val tempdata=data.get(random.nextInt(data.size))
            if(tempdata.eng.equals(eng)||tempdata.kor in str){
                continue
            }
            else{
                str.add(tempdata.kor)
            }
        }
        return str
    }
